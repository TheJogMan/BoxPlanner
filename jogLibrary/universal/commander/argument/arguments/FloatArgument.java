...
{
	@Override
	public ReturnResult<Float> interpret(HoardingIndexer<Character> source, Executor executor)
	{
		String token = StringValue.consumeString(source, ' ').value();
		try
		{
			return new ReturnResult<>(Float.parseFloat(token));
		}
		catch (Exception e)
		{
			return new ReturnResult<>("Not a valid float.");
		}
	}
